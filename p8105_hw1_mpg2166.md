...
The random sample vector `vec_norm` and the logical vector `vec_logic`
work for calculating the mean. The character vector `vec_char` and the
factor vector `vec_factor` do not work.
